...
data StakeParams = StakeParams
    { sCreator        :: !PubKeyHash
    , sSubscriber     :: !PubKeyHash
    , sStake          :: !Integer
 --   , gAsset          :: !POSIXTime
    , sToken          :: !ThreadToken
    } deriving (Show, Generic, FromJSON, ToJSON, Prelude.Eq)

PlutusTx.makeLift ''StakeParams

-- This version considere only Duration and Amount as Parameter
data StakeParams = StakeParams Int Int
    deriving (Show, Generic, FromJSON, ToJSON)


PlutusTx.unstableMakeIsData ''StakeParams


data StakeDatum = StakeDatum (Maybe $ StakeParams Int Int) | Finished
    deriving Show

instance Eq GameDatum where
    {-# INLINABLE (==) #-}
    GameDatum mc    == GameDatum mc' = mc == mc'
    Finished        == Finished          = True
    _               == _                 = False

PlutusTx.unstableMakeIsData ''GameDatum

data GameRedeemer = Stake | Burn | Destroy
    deriving Show
